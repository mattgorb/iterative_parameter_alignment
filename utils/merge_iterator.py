...
class Merge_Iterator:
    def __init__(self, args, train_loaders, test_loader,train_weight_list, device, weight_dir):
        ...
        self.model_cnf_str= f'model_{self.args.model}_ds_{self.args.dataset}_seed_{self.args.seed}_n_cli_{self.args.num_clients}_{uneven}_ds_split_{self.args.dataset_split}_ds_alpha_{self.args.dirichlet_alpha}' \
    ...
    def comparison_statistics(self, iteration):
        ...
        for idx, trainer in enumerate(self.model_trainers):
            ...
            correct=torch.BoolTensor().to(self.args.device)
