...
class Merge_Iterator:
    ...
    def run(self):
        ...
        for iter in range(merge_iterations):
            ...
            if iter>0:
                model1.fc1.weight=nn.Parameter(model2.fc1.weight_align, requires_grad=True).to(self.device)
                model1.fc2.weight=nn.Parameter(model2.fc2.weight_align, requires_grad=True).to(self.device)#.clone().detach()
            model1_trainer.fit()

            if iter>0:
                model2.fc1.weight_align=nn.Parameter(model1.fc1.weight, requires_grad=True).to(self.device)
                model2.fc2.weight_align=nn.Parameter(model1.fc2.weight, requires_grad=True).to(self.device)
