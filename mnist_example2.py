...
def generate_mlc(model1, model2, model_new):
    ...
    for model1_mods, model2_mods, new_model_mods in zip(model1.named_modules(), model2.named_modules(), model_new.named_modules()):
        ...
        if hasattr(m1, "weight") and m1.weight is not None:
            ...
            print(torch.where(mlc_mask==0, 1,0))
            print(torch.sum(torch.where(mlc_mask==0, 1,0)))
            #sys.exit()
            ...
            print(f'Module: {n_new} matching zeros: {int(torch.where(mlc_mask==0, 1,0))}/{torch.numel(mlc)})')
            print(f', %: {int(torch.sum(torch.where(mlc_mask==0, 1,0))) / torch.numel(mlc)}')
            sys.exit()
    return model_new
