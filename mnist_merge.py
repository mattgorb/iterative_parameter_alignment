...
class SubnetLinear(nn.Linear):
    ...
    def forward(self, x):
        ...
        scores_diff=None
        if self.scores_align is not None:
            scores_diff=torch.sum((self.scores-self.scores_align).abs())
        return x, scores_diff
...
class Trainer:
    ...
    def train(self,):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            loss = self.criterion(output, target)+100*sd
            train_loss+=loss
            loss.backward()
            self.optimizer.step()
