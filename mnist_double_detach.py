...
class Merge_Iterator:
    ...
    def run(self):
        ...
        for iter in range(merge_iterations):
            ...
            if iter>0:
                model1.fc1.weight_align=nn.Parameter(model2.fc1.weight.clone().detach().to(self.device), requires_grad=False)
                model1.fc2.weight_align=nn.Parameter(model2.fc2.weight.clone().detach().to(self.device), requires_grad=False)
                model1_trainer.optimizer = optim.Adam(model1.parameters(), lr=self.args.lr)
            model1_trainer.fit()
            if iter>0:
                model2.fc1.weight_align=nn.Parameter(model1.fc1.weight.clone().detach().to(self.device), requires_grad=False)
                model2.fc2.weight_align=nn.Parameter(model1.fc2.weight.clone().detach().to(self.device), requires_grad=False)
