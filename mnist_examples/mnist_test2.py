...
def main():
    ...
    for i in range(350):
        ...
        model_merge.fc1.weight=torch.Parameter(torch.mean(torch.cat([model1.fc1.weight, model2.fc1.weight], 0)), requires_grad=False)
        model_merge.fc1.bias=torch.Parameter(torch.mean(torch.cat([model1.fc1.weight, model2.fc1.bias], 0)), requires_grad=False)

        model_merge.fc2.weight=torch.Parameter(torch.mean(torch.cat([model1.fc2.weight, model2.fc2.weight], 0)), requires_grad=False)
        model_merge.fc2.bias=torch.Parameter(torch.mean(torch.cat([model1.fc2.bias, model2.fc2.bias], 0)), requires_grad=False)
