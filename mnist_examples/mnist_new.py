    ...
    def forward(self, x):
        ...
        if self.weight_align is not None:
            if self.args.align_loss=='ae':
                weights_diff = torch.sum((self.weight - self.weight_align).abs())#.pow(1/1.5)#.pow(1.5)
                #weights_diff = torch.sum((out - F.linear(x, self.weight_align, self.bias)).abs())
                #out=0.5*out+F.linear(x, self.weight_align, self.bias)
            elif self.args.align_loss=='se':
                weights_diff = torch.sum((self.weight - self.weight_align)**2)
            elif self.args.align_loss == 'le':
                weights_diff = torch.sum((self.weight - self.weight_align).abs().pow(1.4))#.pow(1/1.5)
            else:
                sys.exit()
    ...
    def run(self):
        ...
        for iter in range(merge_iterations):
            ...
            for i in [0.1, 0.2, 0.3, 0.5,0.7, 0.8, 0.9]:
                self.model_merged_trainer.model.fc1.weight=nn.Parameter((i*model1.fc1.weight+(1-i)*model2.fc1.weight), requires_grad=False)
                self.model_merged_trainer.model.fc2.weight=nn.Parameter((i*model1.fc2.weight+(1-i)*model2.fc2.weight), requires_grad=False)
