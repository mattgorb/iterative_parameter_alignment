...
def get_datasets(args):
    ...
    if args.baseline:
        ...
        if args.imbalanced:
            ...
            d1=ds1_indices[:int(len(ds1_indices)*p)]+ds2_indices[int(len(ds2_indices)*p):]
            d2=ds1_indices[int(len(ds1_indices)*p):]+ds2_indices[:int(len(ds2_indices)*p)]
            dataset1.data,dataset1.targets = dataset1.data[d1],list(np.array(dataset1.targets)[d1])
            dataset2.data, dataset1.targets = dataset2.data[d2], list(np.array(dataset2.targets)[d2])
